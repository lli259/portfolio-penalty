...
for alg in algorithmNames:
	...
	#can't put in one table, cause every model have different size of x and y
	trainSetSubset=trainSet[trainSet["runtime_"+alg]<TIME_MAX-1]
	trainResultSingle=trainSetSubset.copy()
	print("Training data size: ",alg,len(trainResultSingle))
	trainSet_X=trainSetSubset.ix[:,featureList].values
	trainSet_y=trainSetSubset["runtime_"+alg].values
	#XX,yy used to test model trained on individual x,y
	trainSet_XX=trainSet.ix[:,featureList].values
	trainSet_yy=trainSet["runtime_"+alg].values
	...
	trainResultSingle["DT_"+alg+"_pred"]=y_
	y_=dtModel.predict(trainSet_XX)
	...
	trainResultSingle["RF_"+alg+"_pred"]=y_
	y_=rfModel.predict(trainSet_XX)
	...
	trainResultSingle["kNN_"+alg+"_pred"]=y_
	y_=kNeigh.predict(trainSet_XX)
	...
	trainResultSingle.to_csv("training_result_"+alg+".csv")
